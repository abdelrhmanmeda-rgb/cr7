...
export default function App() {
  ...
  const trackUserAction = async (actionType: string, itemName?: string, price?: string | number) => {
    try {
      await fetch('http://localhost:5000/api/statistics/track', {
        method: 'POST',
  ...
  const handleLike = async (postId: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/blog/${postId}/like`, {
        method: 'POST',
        headers: { 
  ...
  const handleAddComment = async (postId: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/blog/${postId}/comment`, {
        method: 'POST',
        headers: { 
          'Accept': 'application/json',
          'Content-Type': 'application/json' 
